...
def render_notification_control(enabled: bool=True):
    components.html(f'''<div style="font-family:Arial,sans-serif;padding:4px 0"><button id="rz-btn" style="padding:10px 14px;border-radius:10px;border:1px solid #888;background:#fff;cursor:pointer;font-weight:700">🔔 Activer RE-ZERO</button><div id="rz-status" style="margin-top:6px;font-size:13px;opacity:.75"></div></div><script>
...
def notify_opportunity(alert:dict,key:str):
    ...
    components.html(f'''<script>try{{const k={key!r};if('Notification' in window&&Notification.permission==='granted'&&localStorage.getItem('rezero_alert')!==k){{new Notification({title!r},{{body:{body!r},tag:'rezero-opportunity',renotify:true}});localStorage.setItem('rezero_alert',k);}}}}catch(e){{}}</script>''',height=1)
